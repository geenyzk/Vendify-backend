...
While impersonating, the backend blocks profile/password/PIN changes, virtual-account generation, Vendify purchases, betting funding, airtime-to-cash submission, wallet transfers, and withdrawal submission for co-owner sessions. The `owner` role is exempt and retains full customer capabilities while viewing an account. Ending impersonation remains available even if `switch_account` is subsequently removed, provided the original account remains active staff.
