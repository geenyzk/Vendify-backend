...
- Session login is available in the adapter but not called between successful OTP verification and transfer; the pasted documentation does not require an extra login step. As of 2026-09-14 production has never called it and has no successful transfer to compare against, so it was not added. A 3000 whose wording is about the session now logs `session_rejected`, which would be the evidence to revisit this. To test it under control, set `AIRTIME_TO_CASH_AUTOMATION_SESSION_LOGIN_BEFORE_TRANSFER=true` (default false, then `php artisan config:clear`): each PIN submission first calls session login with the verified session and transfers only on a confirmed session, using any new session ID it returns. An unconfirmed login submits no transfer, uses no PIN attempt and credits nothing; a 4010 expires the conversion. The `session` log line reports whether the returned session ID matched, and each `convert` line records whether the flag was on.
