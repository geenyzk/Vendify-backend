...
| 3000 failure | Transfer 3000 is classified by meaning: `invalid_pin` and `insufficient_balance` return to PIN entry on the same session, except that the third `invalid_pin` for a conversion fails it (`pin_attempts_exhausted`) and requires a new conversion; `session_rejected` and `transfer_failed` are terminal. None credits a wallet. The current official page documents low-balance/recharge and invalid-PIN response bodies. Neither matches the controlled production six-word response, so wording is matched loosely and only allowlisted terms are logged |
...
- Controlled production reference `ATC-cfdb37fd-a80d-40c1-8060-a3ce52733a3c` ran quota → OTP → verify → session login → transfer with the Admin setting enabled. Session login returned 2000 and the same valid session ID, but transfer still returned 3000. This rules out a missing pre-transfer session login as the cause of that rejection. The opt-in remains available, but this evidence does not justify further transfer-sequence or session speculation. The six-word transfer message was safely reduced to `* can not transfer * *`; it matches neither official 3000 example, so the three masked words cannot be reconstructed from existing evidence.
